# Puzzle: http://adventofcode.com/2017/day/2
...
def calc_check_value1(row)
  row.max - row.min
end

def find_divisible_pair(row)
  divisible_pair = ->(xs) do
    dividend, *rest = xs
    divisor = rest.find { |x| dividend % x == 0 }
    divisor ? [dividend, divisor] : divisible_pair.call(rest)
  end

  sorted = row.sort { |a,b| b <=> a }
  divisible_pair.call(sorted)
end

def calc_check_value2(row)
  divisible_quotient = ->(xs) do
    dividend, *rest = xs
    quotient, remainder =
      rest.lazy
          .map { |divisor| [dividend / divisor, dividend % divisor] }
          .find { |q,r| r == 0 }
    quotient || divisible_quotient.call(rest)
  end

  sorted = row.sort { |a,b| b <=> a }
  divisible_quotient.call(sorted)
end

checksum1 = spreadsheet.map(&method(:calc_check_value1)).sum

puts "The checksum for Part 1 is #{checksum1}"

checksum2 = spreadsheet.map(&method(:calc_check_value2)).sum

puts "The checksum for Part 2 is #{checksum2}"
